...
async function addRequestToServer(musicRequest, requesterName) {
    try {
        const response = await fetch('https://nodayz.onrender.com/api/requests', {
            method: 'POST',
            headers: {
...
function refreshRequestsList() {
    ...
    requests.forEach(request => {
        ...
        requestElement.innerHTML = `
            <div class="request-timestamp">${timeAgo}</div>
            <div class="request-number">${request.number}. ${request.name}:</div>
            <div class="request-content">${request.music}</div>
        `;
